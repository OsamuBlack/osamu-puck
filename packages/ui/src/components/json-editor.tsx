...
export const JsonEditor = React.memo(function JsonEditorComponent({
  ...
  const applyHighlighting = useMemo(() => {
    return (code: string) => {
      let result = code
        ...
        .replace(/: *"([^"]+)"/g, ': <span className="text-green-600">"$1"</span>')
        // Highlight numbers
        .replace(/: *([0-9]+)/g, ': <span className="text-rose-600">$1</span>')
        // Highlight booleans
        .replace(/: *(true|false)/g, ': <span className="text-yellow-600">$1</span>')
        // Highlight null
  ...
  const actionButtons = useMemo(() => (
    <div className="flex justify-end gap-2 mt-3">
      <Button size="sm" type="button" variant={"ghost"} onClick={handleFormat}>
        Format
      </Button>
      <Button size="sm" type="button" onClick={handleApply} disabled={!isValid}>
        Apply
      </Button>
    </div>
  ...
  return (
    <div className={`w-full font-mono ${className}`}>
      ...
      <div className="relative">
        ...
        <textarea
          ref={editorRef}
          className={`w-full px-[15px] py-[12px] rounded-[4px] font-mono text-sm resize-y focus:outline-none absolute inset-0 bg-transparent z-10 text-transparent caret-foreground border border-solid ${
            isValid
        ...
        <div 
          className={`w-full px-[15px] py-[12px] rounded-[4px] font-mono text-sm whitespace-pre-wrap overflow-hidden min-h-[120px] ${
